...
class isAdminInventoryGabung
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (Auth::user() && Auth::user()->hasRole('Aplikasi Gudang') || Auth::user()->hasRole('Aplikasi Purchasing') || Auth::user()->pegawai->jabatan->nama_jabatan == 'Owner' && $payment_bengkel->status == 'lunas') {
            return $next($request);
        }
        abort(403);
    }
